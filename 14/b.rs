...
fn cost(input: &HashMap<String, Rec>, nfuel: usize) -> usize {
...
fn exp_search<F: Fn(usize) -> Ordering>(test: F) -> usize {
    let mut upper = 1;
    while let Ordering::Less = test(upper) {
        upper *= 2;
    }
...
fn main() {
    ...
    let res = exp_search(|nfuel| cost(&input, nfuel).cmp(&1_000000_000000));
    println!("{res}");
}
